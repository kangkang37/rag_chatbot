...
def addVectorDataToDb(file_path) -> None:
    ...
    # emb = embedding_model(splited_data) # 不要对单个string splited_data【0】向量化
    # print(len(emb))
    # print(len(emb[0])) # !!!对于一个string，emb得到173*384的矩阵
    try:
        emb = embedding_model(splited_data)  # 不要对单个string splited_data【0】向量化
        print(len(emb))
...
def searchDataByVector(query: str):
    try:
        ...
        res = collection.query(
            query_embeddings=query_vector, # 这里用query_vector或者query_vector[0]好像都可以
            n_results=2,
...
if __name__=='__main__':
    file_path= "./pdf_files/resume_KangyiQiu.pdf"
